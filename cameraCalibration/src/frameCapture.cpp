...
#define FRAMERATE 75.0 //Measured from the program
...
IS_WAIT_EVENTS waitFrame = {enabledEvents,
                            sizeof(enabledEvents) / sizeof(int),
                            FALSE,
                            2000 };
...
bool allocateMemory() {
    ...
    printf("Memory allocated with IDs: ");
    for (auto memID : memIdVector) {
        printf("%i,", memID);
    }
    printf("\n");
    printf("Memory addresses: ");
    for (auto memAddress : memPointerVector) {
        printf("%p,", memAddress);
    }
    printf("\n");
...
bool setupCamera() {
    ...
    //areaOfInterest.s32Y = 0; //If lower half    
